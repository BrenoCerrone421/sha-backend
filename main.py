...
print("==============================================")
print("     INICIANDO AGENTE COMERCIAL SHA v3.1      ")
print("==============================================")
...
try:
    ...
    print(f"❌ ERRO CRÍTICO Sheets: {e}", file=sys.stderr)
...
try:
    ...
    print(f"❌ ERRO CRÍTICO Redis: {e}", file=sys.stderr)
...
if GEMINI_API_KEY:
    try:
        ...
        print(f"❌ ERRO CRÍTICO Gemini: {e}", file=sys.stderr)
...
def processar_mensagem_zapi(dados):
    try:
        ...
        prompt_final = (
            ...
            f"Histórico da conversa: {historico}. "
            f"A última pergunta do cliente foi: \"{pergunta_usuario}\". Responda."
        )
...
def enviar_resposta_zapi(numero_destino, texto_da_resposta):
    ...
    try:
        resposta = requests.post(url_api_zapi, json=payload, headers=headers, timeout=10)
...
def receber_mensagem():
    ...
    if request.method == "POST":
        ...
        if not dados_entrada or not dados_entrada.get("instanceId"):
            return "Ignorado", 200
